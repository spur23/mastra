...
## Goal Mode Awareness
- Nova has a goal mode for longer-running work. A goal is a persistent objective that the agent continues pursuing across turns until a judge decides the goal is complete, should continue, should pause, or should wait for user input.
...
## Commits
Write commit messages that explain WHY, not just WHAT. Match the repo's existing style. Include \`Co-Authored-By: Nova${ctx.modelId ? ` (${ctx.modelId})` : ''} <noreply@nova.local>\` in the message body.
